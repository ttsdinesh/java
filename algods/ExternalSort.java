 ...
 * 
 *         Source http://www.ashishsharma.me/2011/08/external-merge-sort.html
 */
public class ExternalSort {
	 ...
	 * and write the result to a bunch of temporary files that have to be merged
	 * later.
	 * 
	 * @param File
	 * @param Comparator
	 * @return List of smaller files containing sorted data
	 * @throws IOException
	 */
		...
		try {
			...
			try {
				while (line != null) {
					...
					while ((currentblocksize < blocksize) && ((line = fbr.readLine()) != null)) { // as long as you have
																									// 2MB
						tmplist.add(line);
						currentblocksize += line.length(); // 2 + 40; // java uses 16 bits per character + 40 bytes of
															// overhead (estimated)
					}
	...
	public static int mergeSortedFiles(List<File> files, File outputfile, final Comparator<String> cmp)
			throws IOException {
		Queue<BinaryFileBuffer> pq = new PriorityQueue<BinaryFileBuffer>(11,
				(BinaryFileBuffer i, BinaryFileBuffer j) -> cmp.compare(i.peek(), j.peek()));
		for (File f : files) {
	...
	public static void main(String[] args) throws IOException {
		...
		Comparator<String> comparator = (String r1, String r2) -> r1.compareTo(r2);
